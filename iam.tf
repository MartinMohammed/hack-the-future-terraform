    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "sts:ExternalId" = "YG35686_SFCRole=3_ot13eeoaBTU7VV02WfeXmVP+Ll4=" # Replace with Snowflake's external ID
          }
        }
      }
    ]
...
resource "aws_s3_bucket_policy" "cloudfront_policy" {
  ...
  policy = data.aws_iam_policy_document.cloudfront_access_s3_static_website_policy.json
}

data "aws_iam_policy_document" "cloudfront_access_s3_static_website_policy" {
  statement {
    principals {
      type        = "*"
      identifiers = ["*"]
